...
export function CampaignDetail() {
  ...
  if (notFound) return <p className="text-slate-500 text-sm">Campaign not found.</p>;
  if (!campaign) return <p className="text-slate-500 text-sm">Loading…</p>;

  return (
    <div className="max-w-3xl space-y-6">
      <div>
        <div className="flex items-center justify-between mb-1">
          <h1 className="text-lg font-semibold text-purple-300">⚠ {campaign.name}</h1>
          <span className="text-sm text-purple-400">{campaign.confidence.toFixed(0)}% confidence</span>
        </div>
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-slate-200">Indicators ({campaign.indicators.length})</h2>
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-slate-200">Organizations involved ({campaign.orgs.length})</h2>
        <div className="space-y-2">
          {campaign.orgs.map((o) => (
